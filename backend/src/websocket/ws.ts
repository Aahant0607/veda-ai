...
export const setupWebSocket = (server: Server) => {
  ...
  wss.on('connection', (ws, req) => {
    ...
    if (!assignmentId) { ws.close(1008, 'Missing assignmentId'); return; }
    ...
    ws.on('close', () => {
      ...
      if (clients.get(assignmentId)?.size === 0) clients.delete(assignmentId);
    });
...
export const notifyClients = (assignmentId: string, payload: object) => {
